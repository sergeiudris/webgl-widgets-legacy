...
var THIS_FOLDER_PATH = ".";
    ...
    function drawScene() {
      ...
      //  console.log(pMatrix);
        ...
        mat4.multiply(mvMatrix, worldRotationMatrix, mvMatrix);
        mat4.multiply(pMatrix, perspectiveRotationMatrix, pMatrix);
    ...
    function initLandoSystem() {
        ...
        for (var i = 0, l = data.length; i < l; i++) {
            ...
            halfWorldWidth = gl.viewportWidth/2;
            halfWorldHeight = gl.viewportHeight / 2;
            halfWorldDepth = gl.viewportDepth / 2;
         
            x = halfWorldWidth - halfWorldWidth / maxValue * element.value;
            y = halfWorldHeight -halfWorldHeight / maxHours * element.hours;
            z = halfWorldDepth- halfWorldDepth / maxAge * element.age;
            if (!(i % 2)) {
                x *= -1;
                y *= -1;
                z *= -1;
            ...
            screenCoords = { x: x, y: y,z:z };
            planet = new Planet(element.text, element.value, [x / Math.abs(worldZ), y / Math.abs(worldZ), z / Math.abs(worldZ)], radius, 0, element.color, screenCoords);// coordsToClip([x,y,z])
            planets.push(planet);
        }
    ...
    function Planet(text,value,position,radius,angle,color,screenCoords) {
        this.text = text;
        this.value = value;
        this.radius = radius;
        this.position = {
    ...
    Planet.prototype.draw = function () {
        ...
        context.fillText(this.value + "\n"+ Math.round(this.position.x)+"\n"+Math.round(this.position.y),resolution.width/2 + this.position.x* Math.abs(worldZ-this.position.z) ,resolution.height / 2 - this.position.y * Math.abs(worldZ -this.position.z));
        mvPushMatrix();
        //mat4.rotate(mvMatrix, degreesToRadians(this.angle), [0.0, 1.0, 0.0]);
       //  console.log(this.position.vec3);
        //var clipCoords = unproject(this.position.vec3, mvMatrix, pMatrix);
        //clipCoords[2] = clipCoords[2]-30;
        var v = this.position.vec3.slice();
        v.push(1);
        mat4.multiplyVec4(mvMatrix, v, v);
    ...
    function unproject(xyz, mvMat,pMat ) {
        var winx = 2 * xyz[0] / gl.viewportWidth - 1;
        var winy = 2 * xyz[1] / gl.viewportHeight - 1;
        var winz = 2 * xyz[2] / gl.viewportDepth - 1;
        var invMat = mat4.create();
        mat4.inverse(pMat, invMat);
        var mvpMat = mat4.create();
        mat4.multiply(mvMat, invMat, mvpMat);
        var n = [winx, winy, winz, 1]
        mat4.multiplyVec4(invMat, n, n);
        return { vector: [n[0] / n[3], n[1] / n[3], n[2] / n[3]], w: n[3] }
    }
    function coordsToClip(v3) {
        var winx = 2 * v3[0] / gl.viewportWidth - 1;
        var winy = 2 * v3[1] / gl.viewportHeight - 1;
        var winz = 2 * v3[2] / gl.viewportDepth - 1;
        var n = [winx, winy, winz, 1];
       // return n;
    ...
    function handleMouseDown(event) {
        ...
        var planet,
            ...
            y
        ;
        x = (lastMouseX - gl.viewportWidth / 2) ;
        y= -(lastMouseY - gl.viewportHeight /2);


        //var newMat = mat4.create();
        //mat4.multiply(mvMatrix,pMatrix, newMat);
      
        //var planet0 = [p.x, p.y, p.z, 1];
       
        //var mv = mat4.create();
        //var per = mat4.create();
        //mat4.transpose(mvMatrix, mv);
        //mat4.transpose(pMatrix, per);

        //mat4.multiplyVec4(mv, planet0, planet0);
        //mat4.multiplyVec4(per, planet0, planet0);

        

        //var mouse = [x / Math.abs(worldZ), y / Math.abs(worldZ), 0 / Math.abs(worldZ), 1];
        var mouse = [x, y, 0, 1];
        var mouseWorldCoords = unproject(mouse, mvMatrix, pMatrix).vector;
        var mouseX = mouseWorldCoords[0];
        var mouseY = mouseWorldCoords[1];
        //mat4.multiplyVec4(mv, mouse, mouse);
        //mat4.multiplyVec4(per, mouse, mouse);
        var i, p, planetCoordsVector, planetWorldCoords, planetX, planetY, w, r;
        for (i = 0, l = planets.length; i < l ; i++) {
             p = planets[i];
             planetCoordsVector = [p.screenCoords.x, p.screenCoords.y, p.screenCoords.z, 1];
             planetWorldCoords = unproject(planetCoordsVector, mvMatrix, pMatrix).vector;
             w = unproject(planetCoordsVector, mvMatrix, pMatrix).w;
             planetX = planetWorldCoords[0];
             planetY = planetWorldCoords[1];
             r =  p.radius/w;
            if (mouseX > planetX -r && mouseX < planetX + r
                && mouseY > planetY - r && mouseY < planetY + r) {
                console.log("planet number "+i+" :")
                console.log(planetWorldCoords);
                console.log(p.radius / w/Math.abs(worldZ));
                console.log(p.value);
                console.log(p.color);
                console.log("-----------------------");
            }
        ...
        // mat4.multiplyVec4(newMat, mouse, mouse);
        //console.log("planet :");
        //console.log(planetCoordsVector);
        //console.log("color: ");
        //console.log(planets[i-1].color);
        //console.log("screenCoords: ");
        //console.log(planets[i-1].screenCoords);
        //console.log("mouseCoords: ");
        //console.log(x, y);
        //console.log("mouse:");
        console.log(mouseWorldCoords);
    ...
    function start() {
        ...
        initTexture();
        loadJSON(THIS_FOLDER_PATH+"/data.json", true, handleLoadedData);
