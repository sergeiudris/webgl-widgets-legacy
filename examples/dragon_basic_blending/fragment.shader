...
vec3 I_diffuse = sourceDiffuseColor*materialAmbientColor*max(0.0,dot(vNormalPosition,sourceDirection)); // change ambientColor to diffuseColor -> different effects
vec3 I_specular  = sourceSpecularColor*materialSpecularColor*pow(max(dot(R,V),0.0),materialShininess);

vec3 I = I_ambient+I_diffuse+I_specular;

gl_FragColor = vec4(I*color, materialAlpha);
}