...
var CANVAS,
    ...
    backgroundProgram,
    programDictionary = {
        attributes: ["aVertexPosition", "aUV", "aNormalPosition"],
...
function start() {
    ...
    Promise.all([
        loadTexture(THIS_FOLDER_PATH + "/dragon.png"),
        loadTexture(THIS_FOLDER_PATH + "/background.png")
    ]).then(function (imgs) {
        dragonTexture = imgs[0];
        backgroundImage = imgs[1];
        initProgram();
...
function drawScene(time) {
    ...
    GL.disableVertexAttribArray(program.aNormalPosition);
    GL.disableVertexAttribArray(program.aVertexPosition);

    GL.useProgram(backgroundProgram);
    GL.uniform1i(backgroundProgram.uSampler, 0);
    GL.enableVertexAttribArray(backgroundProgram.aVertexPosition);


    if (backgroundImage.webglTexture) {
        GL.activeTexture(GL.TEXTURE0);
        GL.bindTexture(GL.TEXTURE_2D, backgroundImage.webglTexture);
    }
    background.draw(GL, backgroundProgram);

    GL.disableVertexAttribArray(backgroundProgram.aVertexPosition);
...
function initProgram() {
    ...
    backgroundProgram = Shadering.createProgram(GL, vSource, fSource, backgroundDictionary);
};
